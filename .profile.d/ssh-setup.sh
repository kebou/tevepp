...
echo ${4LANG_PUBLIC_KEY} > $HOME/.ssh/heroku_id_rsa.pub
chmod 644 $HOME/.ssh/heroku_id_rsa.pub

# Note use of double quotes, required to preserve newlines
echo ${4LANG_PRIVATE_KEY} > $HOME/.ssh/heroku_id_rsa
...
SSH_CMD="ssh -f -p $TUNNEL_PORT -i $HOME/.ssh/heroku_id_rsa -N -L 6472:127.0.0.1:6472 $TUNNEL_USER@$TUNNEL_SITE"
