 ...
 * @param <object> options     Options (language, clientId, apiKey, region, excludePartialMatches, bounds)
 */
var GoogleGeocoder = function GoogleGeocoder(httpAdapter, options) {
  this.options = ['language', 'apiKey', 'clientId', 'region', 'excludePartialMatches', 'channel', 'bounds'];
...
GoogleGeocoder.prototype._geocode = function (value, callback) {
  ...
  var _this = this;
  var bnd = value.withBounds || false;
  var params = this._prepareQueryString(bnd);
  ...
  if (value.address) {
    ...
    params.address = this._encodeSpecialChars(value.text);
  } else {
...
GoogleGeocoder.prototype._prepareQueryString = function (bnd) {
  var params = {
    'sensor': false
  };
  ...
  if (this.options.bounds && bnd) {
    params.bounds = this.options.bounds;
  }
